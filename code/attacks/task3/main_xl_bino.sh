...
# ~20GB GPU memory, ~20-30 min ekstrakcji.
# Strongest universal LLM-detection signal we can run.
...
# Phase 2 + bc + STRONG bino + XL bino (full ladder of LLM-detection signals)
python code/attacks/task3/main.py \
    ...
    --out "$TASK_OUT/submission_xl_bino.csv" \
    --n-rows 2250

echo "Done. Submission at $TASK_OUT/submission_xl_bino.csv"
