#!/bin/bash
#SBATCH --job-name=task3-cross-lm
...
# Cross-LM derived features (OLMo vs Pythia / GPT-2 PPL ratios).
# Uses already-cached PPL features. No new compute, new signal.
# 6 new derived features added to standard stack.
...
python code/attacks/task3/main.py \
    ...
    --out "$TASK_OUT/submission_cross_lm.csv" \
    --n-rows 2250

echo "Done. Submission at $TASK_OUT/submission_cross_lm.csv"
