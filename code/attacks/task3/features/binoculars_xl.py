"""XL Binoculars: Pythia-2.8b (observer) + Pythia-6.9b (performer).

Strongest LLM-detection signal we can run on Jülich (44GB GPU, fp16 ~20GB total).
Pythia-6.9b is 2.5× bigger than 2.8b → much sharper PPL signal for human vs LLM text.
"""
...
OBSERVER_NAME = "EleutherAI/pythia-2.8b"
PERFORMER_NAME = "EleutherAI/pythia-6.9b"
...
def _load() -> None:
    ...
    print(f"  [bino_xl] Loading {OBSERVER_NAME} + {PERFORMER_NAME} on {dev} ({dtype})...")
...
def extract(text: str) -> dict[str, float]:
    ...
    return {
        "bino_xl_score": float(bino_neg),
        "bino_xl_ppl_obs": float(ppl_obs),
        "bino_xl_ppl_per": float(ppl_per),
        "bino_xl_lp_obs": float(mean_lp_obs),
        "bino_xl_lp_per": float(mean_lp_per),
    }
