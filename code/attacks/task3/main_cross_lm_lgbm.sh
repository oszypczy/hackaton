#!/bin/bash
#SBATCH --job-name=task3-clm-lgbm
...
# Cross-LM v1 features + LightGBM (non-linear classifier).
# LogReg captures linear combos of features. LightGBM captures non-linear interactions
# between cross-LM ratios. Different inductive bias may exploit signals differently.
...
python code/attacks/task3/main.py \
    ...
    --out "$TASK_OUT/submission_clm_lgbm.csv" \
    --n-rows 2250

echo "Done. Submission at $TASK_OUT/submission_clm_lgbm.csv"
