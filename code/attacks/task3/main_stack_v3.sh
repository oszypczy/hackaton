#!/bin/bash
#SBATCH --job-name=task3-stack-v3
...
# v3: leak-free + bagging + LightGBM + SelectKBest + extended cross-LM derivations.
# 5 candidate output CSVs: best_base / meta / top3_rank / bagged / lgbm.
...
python code/attacks/task3/stack_v3.py \
    --data-dir "$SCRATCH/llm-watermark-detection" \
    --cache-dir "$TASK_CACHE" \
    --out-dir "$TASK_OUT" \
    --out-prefix "submission_v3"

echo "Done."
ls -la "$TASK_OUT"/submission_v3_*.csv
