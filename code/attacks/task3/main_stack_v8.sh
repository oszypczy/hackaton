#!/bin/bash
#SBATCH --job-name=task3-stack-v8
...
# v8: stable K=40 (avg over 10 MI seeds) + LR + MLP + HGB + multi-seed MLP.
...
python code/attacks/task3/stack_v8.py \
    --data-dir "$SCRATCH/llm-watermark-detection" \
    --cache-dir "$TASK_CACHE" \
    --out-dir "$TASK_OUT" \
    --out-prefix "submission_v8"

echo "Done."
ls -la "$TASK_OUT"/submission_v8_*.csv
