#!/bin/bash
#SBATCH --job-name=task3-stylometric
...
# Stylometric features (CPU-only, fast). 25 features na tekst.
set -euo pipefail
...
python code/attacks/task3/main.py \
    ...
    --out "$TASK_OUT/submission_stylometric.csv" \
    --n-rows 2250

echo "Done. Submission at $TASK_OUT/submission_stylometric.csv"
