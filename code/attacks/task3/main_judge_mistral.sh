#!/bin/bash
#SBATCH --job-name=task3-judge-mistral
...
# LM-judge with Mistral-7B-Instruct.
# OLMo-1B judge → 0.2 leaderboard. 7B model = potentially much stronger.
...
python code/attacks/task3/main.py \
    ...
    --out "$TASK_OUT/submission_judge_mistral.csv" \
    --n-rows 2250

echo "Done. Submission at $TASK_OUT/submission_judge_mistral.csv"
