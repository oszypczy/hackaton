...
| Why do EMAIL and PHONE recover so much better than CREDIT? | They memorise through different signal channels. EMAIL local-parts (`firstname.lastname`) appear repeatedly in the training **text** — the language-modeling loss reinforces them, and the user's name is right there in the question. PHONE is a hybrid: country-code `+1` is text-reinforced (universal across US data, learnt 99.6%), but the last 7 digits are high-entropy and live mostly in the image. CREDIT is image-only-memorised: 16-digit numbers appeared almost exclusively as image pixels in training (the text always read `[REDACTED]`), so the text-loss never reinforced exact digits. With the card image scrubbed, the model has zero text-side signal and falls back to a placeholder. |
| What does "the model placeholder-collapses" mean specifically? | When the model has no signal for a token and no high-confidence completion, it falls back to a low-entropy default — a token sequence that has high marginal probability under the prior but is essentially "I don't know". Concrete shapes we observed: CREDIT collapses to `0000 0000 0000 0000` (39% of rows on blank-image) or arithmetic sequences like `2460 2461 2462 2463`; EMAIL collapses to a small cluster of high-frequency hallucinated domains (`peters.com` / `jones.com` / `lewis.com` cover 75% of bad rows); PHONE collapses to `+1` followed by a few area-code hot-spots from the training distribution (`+1505...`, `+1858...`). Detecting these placeholder shapes is exactly what the `is_dummy` rule does before the CREDIT vote. |
| One-liner you can use if a judge wants the punch line on PII differences? | "EMAIL is text-memorised — the model knows `firstname.lastname` from the question. PHONE is half-text-half-image — it knows `+1` but hallucinates the rest. CREDIT is image-only-memorised — when the card image is scrubbed the model has zero text-side signal and defaults to a placeholder. That's why we route them differently." |
